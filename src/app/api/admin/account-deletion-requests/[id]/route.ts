...
import { z } from "zod";
import { requireAdmin, assertCsrf, reviewAccountDeletionRequest } from "@/modules/identity/service";
...
export const PATCH = withApiHandler(async (request: Request, context: { params: Promise<{ id: string }> }) => {
  ...
  const result = await reviewAccountDeletionRequest(id, admin.id, parsed.data.decision, parsed.data.notes);
  if (!result.success) {
